...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.code == 200) {
      ...
      setStatus({ succes: false, message: 'Algo fue mal, por favor, intenta otra vez más tarde.'});
    }
  };
      ...
      <Container>
        <Row className="align-items-center">
          <Col size={12} md={6}>
            <TrackVisibility>
              {({ isVisible }) =>
                <img className={isVisible ? "animate__animated animate__zoomIn" : ""} src={contactImg} alt="Contacto"/>
              }
            </TrackVisibility>
          </Col>
          <Col size={12} md={6}>
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    <Col size={12} sm={6} className="px-1">
                      <input type="text" value={formDetails.name} placeholder="Nombre Completo" onChange={(e) => onFormUpdate('name', e.target.value)} />
                    </Col>
                    <Col size={12} sm={6} className="px-1">
